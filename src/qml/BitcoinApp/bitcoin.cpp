...
void BitcoinQmlDebugMessageHandler(QtMsgType type, const QMessageLogContext& context, const QString& msg)
{
    Q_UNUSED(context);
    if (type == QtDebugMsg) {
...
int QmlGuiMain(int argc, char* argv[])
{
    ...
    qInstallMessageHandler(BitcoinQmlDebugMessageHandler);
